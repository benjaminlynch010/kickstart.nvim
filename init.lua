...
-- Disable swapfile creation to avoid cluttering directories
vim.opt.swapfile = false

-- Don't create backup files (~file.txt)
vim.opt.backup = false

-- Set directory for persistent undo history
...
-- Enable 24-bit colors for modern terminals
vim.opt.termguicolors = true

-- Keep 8 lines visible above/below cursor while scrolling
vim.opt.scrolloff = 8

-- Show sign column for git/diagnostics markers
vim.opt.signcolumn = 'yes'

-- Display vertical line at column 80 as code width guide
...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      vtsls_server.capabilities = vim.tbl_deep_extend('force', {}, capabilities, vtsls_server.capabilities or {})
      vim.lsp.config('vtsls', vtsls_server)
  ...
  -- Tokyonight colorscheme (kept as backup option)
  {
    'folke/tokyonight.nvim',
    priority = 999, -- Slightly lower priority than rose-pine
    config = function()
      ...
      -- Load the colorscheme here if needed.
  ...
  { -- Highlight, edit, and navigate code
    ...
    init = function()
      -- Parsers to keep installed. On the new `main` branch, `ensure_installed`
      -- is no longer a `setup()` option — you call the install API yourself.
      ...
      -- Only install parsers that aren't already present so we don't
      -- reinstall everything on every startup.
